...
DIRETORIO = "C:\\Users\\Workspace\\Desktop\\TCC\\farol_inteligente\\files\\"

#Conexão com o banco de dados
app = Flask(__name__)
app.config['SQLALCHEMY_TRACK_MODIFICATION'] = True
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:84268426aS$@localhost/tcc'
...
class Transito(db.Model):
    ...
    def to_json(self):
        return {"id": self.id, "qtd": self.qtd, "data": self.data,  "hora": self.hora, "clima": self.clima, "endereco": self.endereco}
...
def climaTempo():
    ...
    clima_url = "http://apiadvisor.climatempo.com.br/api/v1/weather/locale/3477/current?token=" + str(token)
...
def cadastro_test():
    ...
    try:
        ...
        transito = Transito(qtd=body["qtd"], data=body["data"], hora=body["hora"], clima=body["clima"], endereco=body["endereco"])

        db.session.add(transito)
        db.session.commit()

        return gera_response(201, "transito", transito.to_json(), "Criado com sucesso")
...
def atualiza(id):
    ...
    try:
        ...
        db.session.commit()
        return gera_response(200, "transito", transito_objeto.to_json(), "Atualizado com sucesso")
...
def deleta(id):
    ...
    try:
        ...
        db.session.commit()
        return gera_response(200, "transito", transito_objeto.to_json(), "Excluido com sucesso")
...
def post_arquivo():
    ...
    try:
        ...
        transito = Transito(qtd=int(qtd), data=data_hora[1], hora=data_hora[0], clima=str(clima_tempo), endereco=endereco)
